...
class SchoolAdminAttendance extends React.Component {
  ...
  touch=(item,stafMarks)=>{
    if(stafMarks!=null){
      this.props.navigation.navigate('ChooseSec',{itemStaf:{item:item,stafMarks:stafMarks}})
    }else{
  ...
  render() {
    ...
    return (
      <View style={{flex:1,backgroundColor:'#000'}}>
            <Headers navigation={this.props.navigation} name={stafMarks!=null?stafMarks:'ATTENDANCE'}
            ...
            <View style={{flex:1,paddingVertical:50,justifyContent:'center',alignItems:'center'}}>
              ...
              <FlatList
                ...
                renderItem={({item, index})=>(
                  <TouchableOpacity onPress={()=>{this.touch(item,stafMarks)}}
