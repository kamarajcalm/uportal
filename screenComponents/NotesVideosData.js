...
class NotesVideosData extends React.Component {
 ...
 notesData=()=>{
   return(
     <View style={{marginVertical:15,marginHorizontal:10}}>
       <FlatList  data={this.state.notes} numColumns={3} keyExtractor={(item, index) => index.toString()} renderItem={({item, index})=>(
 ...
 videosData=()=>{
     return(
       <View style={{marginVertical:15,marginHorizontal:10}}>
         <FlatList  data={this.state.videos} numColumns={3} keyExtractor={(item, index) => index.toString()} renderItem={({item, index})=>(
           <TouchableOpacity style={{borderRadius:7,marginHorizontal:10}}>
              <View>
               <Image source={(item.img)} style={{height:width*0.3,width:width*0.27,borderRadius:7}}/>
               <Text style={[styles.text,{color:'#fff',fontSize:12,paddingVertical:4,fontWeight:'700',textAlign:'center'}]}>{item.name}</Text>
                      ...
                      return (
                        ...
                        {i==0&&this.state.selectedTab==0&&
                           <View style={{flex:1,}}>
                            <ScrollView>
                              {notes==true &&
                              <View>{this.notesData()}</View>
                              }
                              {videos==true &&
                              <View>{this.videosData()}</View>
                              }
                            </ScrollView>
                           </View>
                        }
